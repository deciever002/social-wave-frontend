...
const Form = ({currentId,setCurrentId}) => {
  ...
  const handleSubmit = (e) => {
    ...
    if(currentId) {
      dispatch(updatePost(currentId,{...postData,name: user?.userName}));
    }else{
      dispatch(createPost({...postData,name: user?.userName},history));
    }
    clear();
  }
  ...
  return (
    <Paper className={classes.paper}>
      <form autoComplete='off' className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant='h6'> {currentId ? 'Editing' : 'Creating'} a Wave</Typography>
        <TextField
          required
